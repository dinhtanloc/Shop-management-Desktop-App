...
import random as rd
import datetime
start_date = datetime.date(2022, 1, 1)  # ví dụ với ngày bắt đầu là 01/01/2022
end_date = datetime.date(2022, 1, 10)   # ví dụ với ngày kết thúc là 10/01/2022

start_time = datetime.time(9, 0)  # 9 giờ sáng
end_time = datetime.time(21, 0)  # 9 giờ tối

# Khoảng thời gian giữa mỗi lần lặp
time_interval = (datetime.datetime.combine(datetime.date.today(), end_time) - datetime.datetime.combine(datetime.date.today(), start_time)) / 9

date_list = []
for i in range((end_date - start_date).days + 1):
    current_date = start_date + datetime.timedelta(days=i)
    for j in range(9):
        current_time = (datetime.datetime.combine(current_date, start_time) + j * time_interval).time()
        date_list.append(datetime.datetime.combine(current_date, current_time))
...
with open('donhang344.csv', mode='w', newline='',encoding='utf-8') as file:
